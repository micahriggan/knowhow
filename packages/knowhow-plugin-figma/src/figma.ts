...
import qs from "qs";
import { PluginBase, PluginMeta, Plugin } from "@tyvm/knowhow";
...
export class FigmaPlugin extends PluginBase implements Plugin {
  ...
  parseNodeIdsFromUrl(url: string): string[] | null {
    ...
    let nodeIds = [];
    if (Array.isArray(nodeParam)) nodeIds = nodeParam;
    if (typeof nodeParam === "string") nodeIds = nodeParam.split(",");
    return nodeIds || null;
  }
  ...
  async embed(userPrompt: string): Promise<MinimalEmbedding[]> {
    ...
    const figmaData = await Promise.all(urls.map((url) => this.loadFigmaData(url)));
  ...
  async call(userPrompt: string): Promise<string> {
    const urls = this.extractUrls(userPrompt);
    if (!urls || urls.length === 0) return "FIGMA PLUGIN: No Figma files found";

    const figmaData = await Promise.all(urls.map((url) => this.loadFigmaData(url)));
    const figmaDataFiltered = figmaData.filter((data) => data !== null);

    if (figmaDataFiltered.length === 0) return "FIGMA PLUGIN: Failed to fetch data for Figma files";

    const responses = [];
    for (const data of figmaDataFiltered) {
      for (const nodeId in data.images) {
        if (!data.images.hasOwnProperty(nodeId)) continue;
        responses.push({ nodeId, imageUrl: data.images[nodeId] });
      }
    }
